...
/** 三栏任务详情页：状态、只读初始纲要、线性进度与最终输出围绕同一任务快照渲染。 */
export function TaskPage({ task, events, error, onCancel, onBack }: { task?: AgentState; events: WorkflowEvent[]; error?: string; onCancel(): Promise<void>; onBack(): void }) {
  ...
  return <main className="task-page"><button onClick={onBack}>← 返回任务列表</button><header><h1>{task.question}</h1><span className="status">{task.status}</span>{!["SUCCESS", "FAILED", "CANCELLED"].includes(task.status) && <button onClick={() => void onCancel()}>取消任务</button>}</header>
    ...
    <div className="columns"><aside className="panel"><h2>任务状态</h2><p>当前阶段：{task.currentNode}</p><p>任务类型：{task.taskType ?? "待分析"}</p></aside>
      <div><PlanPanel task={task} /><TaskResult task={task} /></div><WorkflowProgress task={task} /></div>
